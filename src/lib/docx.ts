...
export function exportMembers(members: Member[], areas: Area[]) {
  ...
  return downloadDoc(
    `oxygen-anggota-${Date.now()}.docx`,
    "Laporan Data Anggota Oxygen.id",
    [
...
export function exportAreas(areas: Area[], members: Member[]) {
  return downloadDoc(
    `oxygen-perumahan-${Date.now()}.docx`,
...
export function exportTreasury(treasury: Treasury[], areas: Area[]) {
  ...
  return downloadDoc(
    `oxygen-kas-rt-${Date.now()}.docx`,
    "Laporan Kas RT Komisi Oxygen.id",
    [
...
export function exportRewards(rewards: Reward[]) {
  return downloadDoc(
    `oxygen-reward-${Date.now()}.docx`,
    "Katalog Reward Warga Oxygen.id",
    [
      makeTable(
...
export function exportLogs(logs: ActivityLog[]) {
  ...
  return downloadDoc(
    `oxygen-activity-log-${Date.now()}.docx`,
    "Riwayat Aktivitas Sistem Oxygen.id",
    [
...
export function exportSummary(
  ...
  return downloadDoc(
    `oxygen-ringkasan-${Date.now()}.docx`,
    "Ringkasan Eksekutif Program Mitra RT Oxygen.id",
    [
