...
export default function App() {
  ...
  // Fetch ONCE on mount — no auto-refresh, no complex subscriptions
...
function MainPage({ shared, onAdminClick, onGalleryClick }: { shared: SharedData; onAdminClick: () => void; onGalleryClick: () => void }) {
  ...
  const handleDonasi = (e: React.FormEvent) => {
    ...
    shared.setTotalDana(prev => prev + jumlah);
    ...
    // 2. BACKGROUND: Save to Supabase (fire-and-forget)
    Promise.resolve(supabase.from('keuangan').insert([{ nama: donorName, jenis: donasiForm.metode === 'cash' ? 'cash' : 'donasi', jumlah, keterangan: `[${metodeLabel}] ${donasiForm.alamat}`, is_anon: donasiForm.isAnon }])).then(({ error }) => {
