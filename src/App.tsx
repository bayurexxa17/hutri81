...
export default function App() {
  ...
  const loginPanitia = () => {
    const pass = panitiaPass.toLowerCase().trim();
    const ownerPasses = ['owner81','supabase81','aulia81','superadmin','owner'];
    const panitiaPasses = ['mawar81','panitia81','81mawar','admin'];
    if (ownerPasses.includes(pass) || pass===getSupabaseConfig().secretKey.toLowerCase()) {
      setIsPanitia(true); setIsOwner(true); setShowPanitiaLogin(false); setPanitiaPass(''); setAdminTab('supabase'); document.getElementById('admin')?.scrollIntoView({behavior:'smooth'});
    } else if (panitiaPasses.includes(pass)) {
      setIsPanitia(true); setIsOwner(false); setShowPanitiaLogin(false); setPanitiaPass(''); setAdminTab('overview'); document.getElementById('admin')?.scrollIntoView({behavior:'smooth'});
    } else alert('Password salah! Panitia: mawar81 / panitia81 | Owner: owner81');
  };
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl border shadow-sm p-5 flex flex-col"><h3 className="font-black text-[14px]">📱 QRIS Donasi Resmi - AULIA KOMARI</h3>
            <div className="mt-4 bg-[#FFF7ED] border-2 border-dashed border-[#C1272D]/30 rounded-2xl p-4 flex flex-col items-center text-center">
              <img src="/images/qris-aulia-komari.png" alt="QRIS AULIA KOMARI ASLI" className="h-64 w-64 object-contain rounded-xl bg-white p-2 border shadow-sm" />
...
alter publication supabase_realtime add table pendaftar, donasi, pendanaan;`}</pre></div>
      ...
      {showPanitiaLogin && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-zinc-900/70 backdrop-blur" onClick={()=>setShowPanitiaLogin(false)} /><div className="relative w-full max-w-[360px] bg-white rounded-[20px] p-6 shadow-2xl"><h3 className="font-black">🔒 Login Panitia</h3><p className="text-[11px] text-zinc-500 mt-1">Panitia: mawar81 / panitia81<br/>Owner: owner81 (akses Supabase)</p><input type="password" value={panitiaPass} onChange={e=>setPanitiaPass(e.target.value)} placeholder="Password" className="mt-4 w-full h-11 px-4 rounded-xl border text-[13px]" /><button onClick={loginPanitia} className="mt-3 w-full h-11 rounded-xl bg-[#C1272D] text-white font-black text-[13px]">Masuk</button></div></div>)}
