...
export default function AdminPage({ onBack, shared }: Props) {
  ...
  const handleSaveKeuangan = (k: KeuanganEntry) => {
    ...
    // Simpan ke Supabase (log error jika gagal)
    Promise.resolve(supabase.from('keuangan').insert([{ nama: k.nama, jenis: k.jenis, jumlah: k.jumlah, keterangan: k.keterangan, is_anon: k.is_anon || false }]))
      .then((res: any) => {
        if (res?.error) console.warn('⚠️ Supabase keuangan insert gagal:', res.error.message, '— Data tersimpan di localStorage.');
        else shared.fetchKeuangan();
