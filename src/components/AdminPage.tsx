...
export default function AdminPage({ onBack, shared }: Props) {
  ...
  const handleSaveKeuangan = (k: KeuanganEntry) => {
    ...
    Promise.resolve(supabase.from('keuangan').insert([{ nama: k.nama, jenis: k.jenis, jumlah: k.jumlah, keterangan: k.keterangan, is_anon: k.is_anon || false }])).then(() => shared.fetchKeuangan()).catch(() => {});
  };
  ...
  const handleDeleteKeuangan = (k: KeuanganEntry) => {
    ...
    if (k.id && k.id > 0) Promise.resolve(supabase.from('keuangan').delete().eq('id', k.id)).then(() => shared.fetchKeuangan()).catch(() => {});
  };
