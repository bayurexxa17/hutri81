...
export default function AdminPage({ onBack, shared }: Props) {
  ...
  const handleSaveKeuangan = (k: KeuanganEntry) => {
    ...
    setDbError('');
    // Simpan ke Supabase — TAMPILKAN error jika gagal
    supabase.from('keuangan').insert([{ nama: k.nama, jenis: k.jenis, jumlah: k.jumlah, keterangan: k.keterangan, is_anon: k.is_anon || false }])
      .then((res) => {
        if (res.error) {
          const msg = `Supabase error: ${res.error.message} (code: ${res.error.code})`;
          setDbError(msg);
          alert(`⚠️ Gagal simpan ke Supabase!\n\n${msg}\n\nKemungkinan:\n1. Tabel 'keuangan' belum dibuat\n2. Kolom tidak cocok\n3. RLS policy belum diset\n\nData tetap tersimpan di localStorage.`);
        } else {
          setDbError('');
